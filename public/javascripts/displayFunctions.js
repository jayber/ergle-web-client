...
function stackFileVersions() {
    $(".eventList").each(function () {
        ...
        $(".eventTitle", this).each(function () {
            var thisTitle = $(this).html();
            var listElement = $(this).parents("ul.eventList li");
        ...
        for (var key in latestVersionMap) {
            if (latestVersionMap.hasOwnProperty(key)) {
                ...
                if (element.children(".sheaf").length == 0) {
                    var target = element.children(".comments");
                    if (target.length==0) {
                        target = element.children().last();
                    } else {
                        target = target.prev();
                    }
                    target.after(render("sheafTemplate"));

                    $(".sheaf", element).click(function (e) {
                        unstackVersions($(this).parent().find(".eventTitle").html(), $(this).parents(".eventList"));
                        return false;
                    });
                }
...
function unstackVersions(title, context) {
    $(".eventTitle", context).each(function () {
        var thisTitle = $(this).html();
        if (title == thisTitle) {
            var listElement = $(this).parents("ul.eventList li");
            listElement.show();
